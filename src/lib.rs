...
pub async fn untar_layers(
    ...
    for path in images.iter() {
        ...
        log.trace(&format!(
            "[untar_layers (lib)] cache file {}",
            cache_file.clone()
        ));
        if !Path::new(&cache_file).exists() {
            let file = format!("{}/{}/{}", blobs_dir.clone(), &blob[0..2], blob);
            log.info(&format!("[untar_layers (lib)] blobs file {}", file));
            ...
            for entry in entries.unwrap() {
                ...
                if path_str.contains("configs/") || path_str.contains("release-manifests/") {
                    ...
                    log.info(&format!(
                        "[untar_layers (lib)] untarring file {} ",
                        &blob[..6]
                    ));
                    let this_tar_gz =
                    ...
                    match this_archive.unpack(cache_file.clone()) {
                        ...
                        Err(error) => {
                            let msg = format!(
                                "[untar_layers (lib)] skipping this error : {} ",
                                &error.to_string()
                            );
                            log.warn(&msg);
                        }
            ...
            log.info(&format!("[untar_layers (lib)] cache exists {}", cache_file));
        }
    }
}
    ...
    match paths {
        Ok(res_paths) => {
            for path in res_paths {
                let entry = path.expect("[find_dir (lib)] could not resolve path entry");
                let file = entry.path();
                // go down one more level
                let sub_paths = fs::read_dir(file).unwrap();
                for sub_path in sub_paths {
                    let sub_entry =
                        sub_path.expect("[find dir (lib)] could not resolve sub path entry");
